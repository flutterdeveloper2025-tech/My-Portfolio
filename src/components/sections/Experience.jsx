...
import {fadeUp,stagger,G,C} from "../../utils/theme";
    ...
    <section id="experience" className="py-24 relative">
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <motion.div ref={ref} initial="hidden" animate={inView?"show":"hidden"} variants={stagger} className="mb-14">
          <motion.p variants={fadeUp} className="font-mono text-sm mb-2" style={{color:"#8b5cf6"}}>{"// work experience"}</motion.p>
          <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl font-black text-white mb-3">
            My <span style={{background:G.purple,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Experience</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-sm" style={{color:"rgba(148,163,184,0.6)"}}>3+ years across university, institute & corporate IT environments.</motion.p>
        </motion.div>
          ...
          <motion.div initial="hidden" animate={inView?"show":"hidden"} variants={stagger} className="space-y-8">
            {EXPERIENCES.map((exp,i)=>(
              <motion.div key={i} variants={fadeUp} className="relative">
                {/* Timeline dot */}
                <div className="absolute -left-12 top-6 w-5 h-5 rounded-full border-2 z-10 flex items-center justify-center"
