...
import {fadeUp,stagger,G} from "../../utils/theme";
    ...
    <section id="projects" className="py-24 relative">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <motion.div ref={ref} initial="hidden" animate={inView?"show":"hidden"} variants={stagger} className="mb-14">
          <motion.p variants={fadeUp} className="font-mono text-sm mb-2" style={{color:"#8b5cf6"}}>{"// featured projects"}</motion.p>
          <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl font-black text-white mb-3">
            My <span style={{background:G.purple,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Projects</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-sm" style={{color:"rgba(148,163,184,0.6)"}}>Real infrastructure, cloud & DevOps projects. View full details on LinkedIn.</motion.p>
        </motion.div>
        ...
        <motion.div initial="hidden" animate={inView?"show":"hidden"} variants={stagger}
          ...
          {PROJECTS.map((p,i)=>(
            <motion.div key={i} variants={fadeUp}
              whileHover={{y:-8,borderColor:`${p.color}40`,boxShadow:`0 20px 60px ${p.color}15`}}
