...
import {fadeUp,scaleIn,stagger,staggerF,G,C} from "../../utils/theme";
    ...
    <section id="skills" className="py-24 relative">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <motion.div ref={ref} initial="hidden" animate={inView?"show":"hidden"} variants={stagger} className="mb-14">
          <motion.p variants={fadeUp} className="font-mono text-sm mb-2" style={{color:"#8b5cf6"}}>{"// skills & expertise"}</motion.p>
          <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl font-black text-white mb-3">Technical <span style={{background:G.purple,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Skills</span></motion.h2>
          <motion.p variants={fadeUp} className="text-sm" style={{color:"rgba(148,163,184,0.6)"}}>3+ years of hands-on expertise across infrastructure, networking, cloud & DevOps.</motion.p>
        </motion.div>
        ...
        <motion.div initial="hidden" animate={inView?"show":"hidden"} variants={stagger}
          ...
          {SKILLS.map((card,ci)=>(
            <motion.div key={ci} variants={scaleIn}
